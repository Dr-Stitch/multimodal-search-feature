...
app.post('/search', upload.single('image'), (req, res) => {
  ...
  const py = spawn('python', ['modular-product-search-agent/search_feature.py']);
  let output = '';

  py.stdin.write(JSON.stringify({ query_text: queryText, query_image_path: imagePath, top_k: topK }));
  py.stdin.end();
